...
import { originStore, getEnvFallbackToken } from "./origin.js";
...
export async function daFetch(path, opts = {}) {
  ...
  // 토큰 해소 순서: 도구 인자 > 연결 헤더(ALS, --http) > DA_AUTH_TOKEN(stdio 만).
  //   인자가 오면 인자가 이긴다. 셋 다 없으면 헤더를 만들지 않는다 — 서버 401 이 답이다.
  //   이 폴백은 여기서만 일어나고 daFetch 는 daApiBase()+path 만 부르므로 DA_API 밖으로 나갈 길이 없다.
  const token = authToken ?? originStore.getStore()?.authToken ?? getEnvFallbackToken();
  const headers = {};
